 ...
 * 但是这种分配不一定是拥有,也可以是排除,主要看分配是使用的{@link PermissionEffect}.
 *
 * @author wuda
 * @since 1.0.0
 */
public interface PermissionGrantManager {
     ...
     * @param permissionEffect {@link Subject}不一定是拥有{@link Target},也有可能是排除
     * @param version 本次权限分配的版本号
     * @param opUserId    操作人用户ID
     */
    void createAssignment(Subject subject, Set<Target> targetSet, PermissionEffect permissionEffect, Long version,Long opUserId);
     ...
     * @param permissionEffect {@link Subject}不一定是拥有{@link Action},也有可能是排除
     * @param version 本次权限分配的版本号
     * @param opUserId    操作人用户ID
     */
    void createAssignment(Subject subject, Target target, Set<Action> actionSet, PermissionEffect permissionEffect,Long version, Long opUserId);

    /**
     * 清除分配给{@link Subject}的{@link Target},分配给{@link Subject}的该{@link Target}上的所有{@link Action}都会被清除.
     * clear assignment与{@link PermissionEffect#DENY}的区别,clear assignment是完全清除掉实体之间的关联,清理后,
     * 实体之间相互独立,没有了任何关联信息.{@link PermissionEffect#DENY}是实体之间建立关联,只是这种关联表明{@link Subject}
     ...
     * <ul>
     * <li>如果想排除掉文件夹b和c呢,这时只需要为用户A分配文件夹b,但是使用{@link PermissionEffect#DENY}即可排除.</li>
     ...
     * @param subject     subject
     * @param target      target
     * @param permissionEffect permissionEffect
     * @return <code>true</code>-如果是
     */
    boolean assignedTargetToSubject(Subject subject, Target target, PermissionEffect permissionEffect);
}
